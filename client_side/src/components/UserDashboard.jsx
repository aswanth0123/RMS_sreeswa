...
const CATEGORIES = [
  {
    label: 'Gold',
    items: ['Rings', 'Necklaces', 'Bangles', 'Chains', 'Bracelets', 'Pendants'],
  },
  {
    label: 'Diamond',
    items: ['Rings', 'Earrings', 'Pendants', 'Necklaces', 'Bracelets'],
  },
  {
    label: 'Platinum',
    items: ['Bands', 'Chains', 'Bracelets'],
  },
  {
    label: 'Silver',
    items: ['Coins', 'Chains', 'Bracelets'],
  },
];
...
const CategoryBar = () => {
  ...
  return (
    <div
      className="w-full bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 sticky top-16"
      style={{
        position: "sticky",
        top: "4rem",
        zIndex: 2000,
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" style={{ zIndex: 45, overflow: 'visible' }}>
        <div className="flex items-center justify-center gap-2 py-3 overflow-x-auto hide-scrollbar" style={{ overflowY: 'visible' }}>
          {CATEGORIES.map(cat => (
            <div
              key={cat.label}
              className="relative shrink-0"
              style={{ zIndex: activeDropdown === cat.label ? 1001 : 'auto' }}
            ...
            >
              ...
              {(activeDropdown === cat.label) && (
                <div
                  className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-2xl p-4 grid grid-cols-2 gap-3 min-w-[240px]"
                  style={{
                    zIndex: 9999,
                    position: "absolute",
                  }}
...
>        <HeroCarousel />
        ...
        <section className="bg-gradient-to-r from-yellow-50 via-amber-50 to-orange-50 rounded-2xl p-8 sm:p-12 shadow-lg">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
              Shine Out Loud
            </h2>
            <p className="text-lg text-gray-700 mb-8">
              Step into the spotlight with vivid designs and timeless craftsmanship across gold, diamond and platinum collections.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="px-8 py-3 bg-gradient-to-r from-yellow-500 to-amber-500 text-white font-semibold rounded-lg hover:from-yellow-600 hover:to-amber-600 transition-all shadow-lg hover:shadow-xl transform hover:scale-105">
                Shop Now
              </button>
              <button className="px-8 py-3 bg-white text-gray-900 font-semibold rounded-lg hover:bg-gray-50 transition-all border-2 border-gray-200 shadow-md hover:shadow-lg">
                Explore Collections
              </button>
            </div>
          </div>
